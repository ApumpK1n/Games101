...
camera = t3.Camera()
camera.ctl.set(pos=[boundary[0] / 2 + 1 , 10 , -40 ],target=[boundary[0] / 2 + 1 , 5 , 0 ],up=[0,1,0])
scene.add_camera(camera)

model = t3.ScatterModel(num_particles)
model.pos = positions
model.radius = radius
...
while gui.running:
    ...
    gui.circles(T(positions.to_numpy()), radius=3, color=0x66ccff)
